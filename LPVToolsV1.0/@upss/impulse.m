...
% IMPULSE  Pointwise impulse response for UPSS objects
%
% IMPULSE(SYS) plots the impulse response for SYS at each point in the 
% domain of SYS. 
%
% [Y,T,X] = IMPULSE(SYS) returns the simulated response data.
...
% in the domain.
%
% [Y,T,X] = IMPULSE(SYS,TFINAL) simulates the impulse responses from t=0 to 
% the final time t=TFINAL.
%
% [Y,T,X] = IMPULSE(SYS,T) uses the user-supplied time vector T for the 
% simulations.
%
% IMPULSE(SYS1,SYS2,...,T) plots the responses of multiple systems on a
% single plot.
%
...
if nout >0
    ...
    if nin <2
        ...
        Tin = varargin{2};
        if isscalar(Tin)
            % Calling: impulse(SYS,TEND)
            flg = true;            
        else
            % Calling: impulse(SYS,T)
    ...
    if flg
        % User supplied a TEND. Must force all impulse simulation to use
        ...
        % Simulate LTI impulse response of system with fastest pole
        if isempty(Tin)
            [~,Tnew,~] = impulse(fastest_sys);  
        else            
            TEND = lpvmax(Tin);
            [~,Tnew,~] = impulse(fastest_sys,TEND.Data);    
        end
    ...
    for k =1:npts
        ...
        [Y(:,:,:,k),T(:,:,k),X(:,:,:,k)] = impulse(function_argument_at_k{:});                   
...
% nout = nargout;
% if nout>0
%    strerr = ' with a UPSS cannot be called with output arguments.';
