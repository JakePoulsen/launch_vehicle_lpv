...
% SUBSREF  Subscripted reference for PSTRUCT objects.
%
% See also: subsref, subsasgn.

switch L(1).type
    case '.'
        if isfield(m.Data,L(1).subs)
            m = lpvgetfield(m,L(1).subs);
        else
            try
                if strncmp(L(1).subs,'value',3)
                    ...
                    if all( allidx>0 )
                        ...
                        m2 = permute(m2,[1 2 3+nv:2+nv+nad 3:2+nv]);
                    else
                        error('The ''value'' method can not interpolate between IV points.')
                    end
                    m = m2;
                    L(2) = [];
